...
# Defer manager imports so module loads even if docker/requests missing (e.g. on Railway)
DockerContainerManager = None
RailwayContainerManager = None
ContainerException = None


def _ensure_managers():
    """Lazily import container managers; required for module.load to exist before deps load."""
    global DockerContainerManager, RailwayContainerManager, ContainerException
    if ContainerException is not None:
        return
    from .container_manager import DockerContainerManager as _Docker, ContainerException as _Exc
    from .railway_manager import RailwayContainerManager as _Railway
    DockerContainerManager = _Docker
    RailwayContainerManager = _Railway
    ContainerException = _Exc
...
def load(app: Flask):
    ...
    backend = container_settings.get("container_backend", "docker")
    # Proxy so we can swap managers when settings change (Docker <-> Railway)
    class _ManagerProxy:
    ...
    _current_manager = (
        RailwayContainerManager(container_settings, app)
        if backend == "railway"
        else DockerContainerManager(container_settings, app)
    )
    container_manager = _ManagerProxy(_current_manager)

    def _update_container_manager(backend, settings):
        nonlocal _current_manager
        if backend == "railway":
            _current_manager = RailwayContainerManager(settings, app)
        else:
            _current_manager = DockerContainerManager(settings, app)
        container_manager._set_manager(_current_manager)
    ...
    def kill_container(container_id):
        ...
        try:
            ...
            return {"error": "Container backend is not initialized. Please check your settings."}
    ...
    def renew_container(chal_id, xid, is_team):
        ...
        return {"success": "Container renewed", "expires": running_container.expires, "hostname": hostname, "port": running_container.port, "connect": challenge.ctype}
    ...
    def create_container(chal_id, xid, uid, is_team):
        ...
        if running_container:
            # Check if container is still running before returning it
            try:
        ...
        # Run a new container (Docker or Railway)
        try:
        ...
        # create_container returns (container_id, hostname, port)
        container_id, hostname, port = result
        ...
        port_int = int(port) if str(port).isdigit() else port
        ...
        return json.dumps({
            "status": "created",
            "hostname": hostname or container_manager.settings.get("docker_hostname", ""),
            "port": port,
            "connect": challenge.ctype,
            "expires": expires
        })
    ...
    def _get_or_create_setting(key, value):
        ...
        if row is None:
            row = ContainerSettingsModel(key=key, value=value or "")
            db.session.add(row)
        else:
            row.value = value or ""
        return row
    ...
    def route_update_settings():
        ...
        backend = request.form.get("container_backend", "docker")
        ...
        if backend == "docker":
            ...
            if not request.form.get("railway_api_token"):
                return {"error": "Railway API token is required"}, 400
            if not request.form.get("railway_project_id"):
                return {"error": "Railway Project ID is required"}, 400
            if not request.form.get("railway_environment_id"):
                return {"error": "Railway Environment ID is required"}, 400

        _get_or_create_setting("container_backend", backend)
        _get_or_create_setting("container_expiration", request.form.get("container_expiration"))
        _get_or_create_setting("docker_base_url", request.form.get("docker_base_url", ""))
        _get_or_create_setting("docker_hostname", request.form.get("docker_hostname", ""))
        _get_or_create_setting("container_maxmemory", request.form.get("container_maxmemory", ""))
        _get_or_create_setting("container_maxcpu", request.form.get("container_maxcpu", ""))
        _get_or_create_setting("railway_api_token", request.form.get("railway_api_token", ""))
        _get_or_create_setting("railway_project_id", request.form.get("railway_project_id", ""))
        _get_or_create_setting("railway_environment_id", request.form.get("railway_environment_id", ""))

        db.session.commit()

        new_settings = settings_to_dict(ContainerSettingsModel.query.all())

        if backend == "docker" and new_settings.get("docker_base_url"):
            _update_container_manager("docker", new_settings)
            try:
            ...
            _update_container_manager(backend, new_settings)
